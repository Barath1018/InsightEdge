...
class DataAnalysisService {
    ...
    static generateNotifications(data, kpis) { const notifications = []; const revenueKPI = kpis.find((k) => k.title === 'Total Revenue'); if (revenueKPI && revenueKPI.trend === 'up')
        notifications.push({ title: 'Revenue Growth Detected', description: `Your revenue has increased by ${revenueKPI.change}. Keep up the great work!`, time: 'Just now', type: 'success', priority: 'high' }); const expenseKPI = kpis.find((k) => k.title === 'Total Expenses'); if (expenseKPI && expenseKPI.trend === 'up')
        notifications.push({ title: 'Expense Increase Alert', description: 'Expenses are trending upward. Consider reviewing cost management strategies.', time: '2 hours ago', type: 'warning', priority: 'medium' }); if (data.length > 1000)
        notifications.push({ title: 'Large Dataset Processed', description: `Successfully analyzed ${data.length.toLocaleString()} records.`, time: '5 minutes ago', type: 'info', priority: 'low' }); return notifications; }
    static generateReports(data) { const reports = []; const revenueColumns = this.findColumns(data, ['revenue', 'sales', 'income', 'amount']); const customerColumns = this.findColumns(data, ['customers', 'clients', 'orders']); if (revenueColumns.length > 0)
        reports.push({ name: 'Revenue Analysis Report', date: new Date().toISOString().split('T')[0], type: 'Financial', status: 'Final', summary: `Comprehensive analysis of ${data.length} revenue records` }); if (customerColumns.length > 0)
        reports.push({ name: 'Customer Behavior Report', date: new Date().toISOString().split('T')[0], type: 'Customer', status: 'Final', summary: `Insights from ${data.length} customer interactions` }); reports.push({ name: 'Data Quality Assessment', date: new Date().toISOString().split('T')[0], type: 'Operations', status: 'Final', summary: 'Analysis of data completeness and accuracy' }); reports.push({ name: 'Performance Metrics Summary', date: new Date().toISOString().split('T')[0], type: 'Analytics', status: 'Final', summary: 'Key performance indicators and trends' }); reports.push({ name: 'Strategic Recommendations', date: new Date().toISOString().split('T')[0], type: 'Strategy', status: 'Draft', summary: 'Actionable insights for business improvement' }); return reports; }
    static generateGrowthAlert(kpis) { const revenueKPI = kpis.find(k => k.title === 'Total Revenue'); const profitKPI = kpis.find(k => k.title === 'Net Profit'); if (revenueKPI && profitKPI && revenueKPI.trend === 'up' && profitKPI.trend === 'up')
        return { title: 'Strong Growth Momentum', description: 'Both revenue and profit are showing positive trends. Your business is performing excellently!', type: 'positive' };
    else if (revenueKPI && revenueKPI.trend === 'up')
        return { title: 'Revenue Growth', description: 'Revenue is trending upward. Focus on maintaining this momentum.', type: 'positive' };
    else
        return { title: 'Performance Review Needed', description: 'Some metrics need attention. Consider reviewing your business strategies.', type: 'neutral' }; }
    static generateDataSummary(data, cleanedData) { const missingData = data.data.length - cleanedData.length; const duplicateRecords = this.countDuplicates(data.data); let dataQuality; const qualityScore = (cleanedData.length / data.data.length) * 100; if (qualityScore >= 95)
        ...
        dataQuality = 'poor'; return { totalRecords: data.data.length, dateRange: this.calculateDateRange(data.data), dataQuality, missingData, duplicateRecords }; }
    static generateInsights(data, kpis) { const insights = []; const revenueKPI = kpis.find(k => k.title === 'Total Revenue'); if (revenueKPI && revenueKPI.trend === 'up')
        insights.push({ title: 'Revenue Growth Opportunity', description: 'Revenue is increasing. Consider scaling successful strategies.', impact: 'high', category: 'revenue' }); const expenseKPI = kpis.find(k => k.title === 'Total Expenses'); if (expenseKPI && expenseKPI.trend === 'up')
        insights.push({ title: 'Cost Management Focus', description: 'Expenses are rising. Review cost structure and identify optimization opportunities.', impact: 'medium', category: 'costs' }); if (data.length > 1000)
        insights.push({ title: 'Data-Driven Decision Making', description: 'Large dataset available. Leverage analytics for strategic decisions.', impact: 'high', category: 'operations' }); return insights; }
    static findColumns(data, keywords, preferred) { if (data.length === 0)
        return []; const headers = Object.keys(data[0]); if (preferred && headers.some(h => h.toLowerCase() === preferred.toLowerCase()))
        return [headers.find(h => h.toLowerCase() === preferred.toLowerCase())]; return headers.filter(header => keywords.some(keyword => header.toLowerCase().includes(keyword.toLowerCase()))); }
    static calculateTotal(data, columns) { if (columns.length === 0)
        return 0; return data.reduce((sum, row) => { const rowSum = columns.reduce((colSum, col) => { const value = row[col]; return colSum + (typeof value === 'number' ? value : 0); }, 0); return sum + rowSum; }, 0); }
    static formatCurrency(value) { return new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD', minimumFractionDigits: 0, maximumFractionDigits: 0 }).format(value); }
    static countDuplicates(data) { const seen = new Set(); let duplicates = 0; data.forEach(row => { const key = JSON.stringify(row); if (seen.has(key))
        duplicates++;
    else
        seen.add(key); }); return duplicates; }
    static calculateDateRange(data) { const dateColumns = this.findColumns(data, ['date', 'created', 'timestamp']); if (dateColumns.length === 0)
        return 'Unknown'; const dates = data.map(row => { const dateValue = row[dateColumns[0]]; if (dateValue) {
        const date = new Date(dateValue);
        return isNaN(date.getTime()) ? null : date;
    } return null; }).filter(d => d !== null); if (dates.length === 0)
        return 'Unknown'; const minDate = new Date(Math.min(...dates.map(d => d.getTime()))); const maxDate = new Date(Math.max(...dates.map(d => d.getTime()))); return `${minDate.toLocaleDateString()} - ${maxDate.toLocaleDateString()}`; }
}
