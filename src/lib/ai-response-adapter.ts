...
export function normalizeGeminiResponse(raw: any): NormalizedAnalysis {
  ...
  const candidates: Partial<NormalizedAnalysis> = { kpis: [], insights: [], charts: [], answer: undefined };
  ...
  if (raw && typeof raw.answer === 'string') {
    ...
    if (fromAnswer) {
      raw = { ...raw, ...fromAnswer };
      if (typeof fromAnswer.answer === 'string') candidates.answer = fromAnswer.answer;
    } else {
      candidates.answer = raw.answer;
    }
  }
  ...
  if ((candidates.kpis?.length || 0) === 0 && (candidates.insights?.length || 0) === 0 && (candidates.charts?.length || 0) === 0) {
    ...
    for (const t of textSources) {
      ...
      if (j) {
        ...
        if (typeof j.answer === 'string') candidates.answer = j.answer;
        break;
      }
    }
  }
  ...
  try {
    NormalizedSchema.parse({ kpis: normalized.kpis, insights: normalized.insights, charts: normalized.charts, answer: normalized.answer });
  } catch (e) {
